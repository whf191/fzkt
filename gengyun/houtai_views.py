...
def zhishidian_fayan(request):
    ...
    if request .method == "GET":
        return render_to_response("houtai_admin_zhishidian_fayan.html",context_instance=RequestContext(request))
        ...
        try:
            ...
            gy_pk = zhongzi.objects.get(pk=gengyuns_pk)

            gys = zhishidian(zhongzi_id=gy_pk,name=name, text=text,create_date=datetime.datetime.today())
            gys.save()
        ...
        if t == "ajax":
            return HttpResponse("0|/zhongzi_open/?pk=%s" % gengyuns_pk)

        else:
            return HttpResponseRedirect("/zhongzi_open/?pk=%s" % gengyuns_pk)
